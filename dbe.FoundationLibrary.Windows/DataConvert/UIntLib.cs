...
{
    /// <summary>
    /// UInt类型(32位有符号整数，范围0 ~ 2E32-1)数据转换类
    /// </summary>
    ...
    public class UIntLib
    {
        ...
        [Obsolete("该方法已弃用，byte[].ToUInt()取代它")]
        public static uint GetUIntFromByteArray(byte[] value, int startIndex = 0, Endianness dataFormat = Endianness.ABCD)
        {
            byte[] data = ByteArrayLib.Get4BytesFromByteArray(value, startIndex, dataFormat);
        ...
        [Obsolete("该方法已弃用，byte[].ToUIntArray()取代它")]
        public static uint[] GetUIntArrayFromByteArray(byte[] value, Endianness dataFormat = Endianness.ABCD)
        {
        ...
        [Obsolete("该方法已弃用，string.ToUIntArray()取代它")]
